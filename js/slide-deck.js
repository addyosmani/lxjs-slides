...
SlideDeck.prototype.addEventListeners_ = function() {
  ...
  //     this.enableSlideDataSrcElements_(this.curSlide_);
  // }.bind(this), false);

  // document.addEventListener('slideenter', function(e) {
  //   var slide = e.target;
  //   window.setTimeout(function() {
  //     this.enableSlideDataSrcElements_(e.slideNumber);
  //     this.enableSlideDataSrcElements_(e.slideNumber + 1);
  //   }.bind(this), 300);
  // }.bind(this), false);
};
...
SlideDeck.prototype.onBodyKeyDown_ = function(e) {
  // Match to prevent typing in hangouts widget from advancing.
  if (e.target.localName == 'hangout-module') {
    return;
  }
...
// 
// this.enableSlideDataSrcElements_(curSlide - 1); // Previous slide.
// this.enableSlideDataSrcElements_(curSlide + 1); // Current slide.
// this.enableSlideDataSrcElements_(curSlide + 2); // Next slide.

   // Enable current slide's [data-src] elements (needed for page load at current slide).
   this.enableSlideDataSrcElements_(curSlide + 1);
   ...
   window.setTimeout(this.enableSlideDataSrcElements_.bind(this, curSlide + 2), 1000);
 ...
 * @param {number} slideNo
 */
SlideDeck.prototype.enableSlideDataSrcElements_ = function(slideNo) {
  var el = this.slides[slideNo - 1];
  if (!el) {
    return;
  }

  var els = el.querySelectorAll('[data-src]');
  for (var i = 0, el; el = els[i]; ++i) {
    this.enableDataSrc_(el);
  }
};
 ...
 * @param {number} slideNo
 */
SlideDeck.prototype.enableDataSrc_ = function(el) {
  var src = el.dataset.src;
  // Use el.getAttribute('src') instead of el.src b/c browser appends
  // hostname to .src. String comparison will fail.
